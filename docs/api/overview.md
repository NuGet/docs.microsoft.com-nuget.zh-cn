...
有关可用的 nuget.exe 版本的简单枚举和下载, 请参阅[node.js 终结点](tools-json.md)。
...
服务索引架构版本由服务索引中的`version`属性表示。 API 规定版本字符串具有的`3`主要版本号。 由于对服务索引架构进行了非重大更改, 将增加版本字符串的次要版本。
...
> [!Note]
> 当源未实现`SearchAutocompleteService`时, 应妥善禁用自动完成行为。 未`ReportAbuseUriTemplate`实现时, 官方 nuget 客户端将回退到 NuGet。组织的报表滥用 URL (由[NuGet/Home # 4924](https://github.com/NuGet/Home/issues/4924)跟踪)。 其他客户端可能选择只是不向用户显示报表滥用 URL。

### <a name="undocumented-resources-on-nugetorg"></a>Nuget.org 上未记录的资源

Nuget.org 上的 V3 服务索引有一些未在上面列出的资源。 不记录资源的原因有很多。

首先, 我们不会将使用的资源记录为 nuget.org 的实现细节。`SearchGalleryQueryService`属于此类别。 [NuGetGallery](https://github.com/NuGet/NuGetGallery)使用此资源将一些 V2 (OData) 查询委托给搜索索引, 而不是使用数据库。 之所以引入此资源是为了实现可伸缩性的原因, 不能供外部使用。
...
向`GET` API 终结点发出的任何请求可能会返回 HTTP 重定向 (301 或 302)。 客户端应通过观察`Location`标头并发出后续`GET`的来正常处理此类重定向。 与特定终结点相关的文档将不会显式调用可以使用重定向的位置。
...
X-NuGet-Client-Version   | **弃用**并替换`X-NuGet-Protocol-Version`
X-NuGet-Protocol-Version | 仅在某些情况下在 nuget.org 上是必需的, 请参阅[nuget.org 协议](NuGet-Protocols.md)
X-NuGet-Session-Id       | *可选*。 NuGet 客户端 4.7 + 识别属于同一 NuGet 客户端会话的 HTTP 请求。

对于`X-NuGet-Session-Id`与中`PackageReference`的单个还原相关的所有操作, 都有一个值。 对于其他方案, 如 "自动`packages.config`完成" 和 "还原", 可能有几个不同的会话 ID, 原因是代码的分解方式。
