...
此 API 由 NuGet 客户端在 Visual Studio、nuget.exe 和 .NET CLI 中用来执行 NuGet 操作，例如[`dotnet restore`](/dotnet/core/tools/dotnet-restore?tabs=netcore2x)、在 VISUAL Studio UI 中搜索和[`nuget.exe push`](../reference/cli-reference/cli-ref-push.md)。
...
> [!Note]
> 如果源未实现 `SearchAutocompleteService` 应正常禁用任何自动完成行为。 如果未实现 `ReportAbuseUriTemplate`，官方 NuGet 客户端将回退到 nuget。组织的报表滥用 URL （由[NuGet/Home # 4924](https://github.com/NuGet/Home/issues/4924)跟踪）。 其他客户端可能选择只是不向用户显示报表滥用 URL。
...
X-NuGet-Client-Version   | **弃用**并替换为 `X-NuGet-Protocol-Version`
