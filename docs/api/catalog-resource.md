...
commitTimeStamp | string           | 是      | 最近提交的时间戳
...
名称            | 类型    | 必须 | 注释
--------------- | ------- | -------- | -----
@id             | string  | 是      | 要提取目录页的 URL
commitId        | string  | 是      | 与此页中的最新提交关联的唯一 ID
commitTimeStamp | string  | 是      | 此页中的最新提交的时间戳
...
弃用             | object                     | 否       | 与包关联的弃用 (与 [包元数据资源相同的格式](registration-base-url-resource.md#package-deprecation)) 
...
packageHash             | string                     | 是      | 包哈希，使用[标准基 64](https://tools.ietf.org/html/rfc4648#section-4)编码
