...
很高兴有关这些更改对包还原，因为它引入了新的工作流。 如果你想要忽略从源代码管理包你只需不要分配出`packages`文件夹。 打开并生成解决方案的 visual Studio 用户会自动还原的包。 对于命令行生成，只需调用`nuget.exe restore`之前调用`msbuild`。 你将不再需要请记得对解决方案，使用"启用 NuGet 包还原"笔势，我们将不再需要修改你的项目以 alter 生成。 这还会生成包含 MSBuild 的导入，尤其是通过 NuGet 的最新功能添加导入的包提供大大提高的体验和[自动导入属性/目标文件](../release-notes/nuget-2.5.md#automatic-import-of-msbuild-targets-and-props-files)\build 文件夹中。
