...
1. <a name="implicit_reference"></a>某些包的**更新**按钮处于禁用状态，并显示一条消息，指出，它"隐式引用由 SDK"（或"AutoReferenced"）。 消息指示包，如 Microsoft.NETCore.App 或 Microsoft.NETStandard.Library，是更大的框架或 SDK 的一部分，并且不应单独更新。 (此类包内部标记有`<IsImplicitlyDefined>True</IsImplicitlyDefined>`。)若要更新的包，更新其所属的 SDK。
