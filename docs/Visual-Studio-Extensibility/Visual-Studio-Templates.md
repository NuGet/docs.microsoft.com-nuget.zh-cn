...
创建项目或项时，Visual Studio 项目和项模板通常需要确保已安装某些包。 例如，ASP.NET MVC 3 模板安装 jQuery、Modernizr 和其他包。

若要支持此功能，模板作者可指示 NuGet 安装必需的包，而不是单独的库。 此后，开发人员可随时轻松更新这些包。

若要了解有关创作模板本身的详细信息，请参阅[在 Visual Studio 中创建项目和项模板](https://msdn.microsoft.com/library/s365byhx.aspx)或[使用 Visual Studio SDK 创建自定义项目和项模板](https://msdn.microsoft.com/library/ff527340.aspx)。
...
实例化模板后，会调用[模板向导](https://msdn.microsoft.com/library/ms185301.aspx)加载要安装的包列表以及有关在何处查找这些包的信息。 包可嵌入 VSIX，嵌入模板，或者位于本地硬盘上（在这种情况下，请使用注册表项引用文件路径）。 本节稍后将提供这些位置的详细信息。

预安装的包使用[模板向导](http://msdn.microsoft.com/library/ms185301.aspx)工作。 模板实例化后会调用特殊向导。 该向导会加载需要安装的包列表，并将该信息传递到相应的 NuGet API。

将包添加到模板的步骤：

1. 在 `vstemplate` 文件中，通过添加 [`WizardExtension`](http://msdn.microsoft.com/library/ms171411.aspx) 元素向 NuGet 模板向导添加引用：
...
建议用于 Visual Studio 项目/项模板的部署方法是 [VSIX 扩展](http://msdn.microsoft.com/library/ff363239.aspx)，因为借助它，可将多个项目/项模板打包在一起，而且开发人员可使用 VS 扩展管理器或 Visual Studio 库轻松发现模板。 也可使用 [Visual Studio 扩展管理器自动更新机制](http://msdn.microsoft.com/library/dd997169.aspx)轻松部署扩展更新。
...
1. 修改 `.vstemplate` 文件中的 `<packages>` 元素，如下所示：
    ...
    `repository` 属性将存储库类型指定为 `extension`，而 `repositoryId` 是 VSIX 本身的唯一标识符（此值为扩展的 `vsixmanifest` 文件中的 [`ID` 属性](http://msdn.microsoft.com/library/dd393688.aspx)）。

1. 将 `nupkg` 文件放置于 VSIX 中名为 `Packages` 的文件夹中。
1. 在 `source.extension.vsixmanifest` 文件中添加必要的包文件作为[自定义扩展内容](http://msdn.microsoft.com/library/dd393737.aspx)。 如果使用的是 2.0 架构，应显示如下：
...
1. 通过在 `.vstemplate` 文件中设置 [`<PromptForSaveOnCreation>`](http://msdn.microsoft.com/library/twfxayz5.aspx)，要求在创建时保存项目/项模板。
