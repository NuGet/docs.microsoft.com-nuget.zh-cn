...
NuGet 工具 Visual Studio 扩展（*nuget*）要求：
...
* [添加包提取 bin not package reference （CTP1）](http://nuget.codeplex.com/workitem/442)
...
* [允许在 NuPack 中替代 BasePath](http://nuget.codeplex.com/workitem/222)
...
* [在生成中设置 VSIX 和 NuPack 的版本号。](http://nuget.codeplex.com/workitem/134)
...
* 将包源从 ATOM 切换到 OData 服务终结点：如果升级到 CTP2 版本的 NuGet，请确保将以下 URL 添加为包源： `https://feed.nuget.org/ctp2/odata/v1/`。
* 已将添加包命令重命名为*安装包*。
...
此生成不适用于旧的 `.nupkg` 文件。 如果收到空引用异常，则使用的是旧的 `.nupkg` 文件，需要使用更新后的[NuGet 命令行工具](http://nuget.codeplex.com/releases/52017/download/165468)重新生成它。
...
* [ILMerge nupack 版本](http://nuget.codeplex.com/workitem/19)
...
* [在仅包含一个 nuspec 文件的文件夹中运行 nupack 应使用该 nuspec](http://nuget.codeplex.com/workitem/52)
...
* [添加包引用 &ldquo;&hellip;&rdquo; 出现在已安装引用的上下文菜单上](http://nuget.codeplex.com/workitem/85)
* [从控制台更新包后，它会将旧版本和新版本显示为已安装](http://nuget.codeplex.com/workitem/86)
* [使用对话框时，控制台中的活动会在使用后消失](http://nuget.codeplex.com/workitem/87)
* [清除 nupack 中的命令行分析](http://nuget.codeplex.com/workitem/89)
...
* [未安装的包的删除包性能错误](http://nuget.codeplex.com/workitem/117)
...
* [NuPack 不应允许空的 &lt;licenseurl&gt; 元素](http://nuget.codeplex.com/workitem/152)
* [重命名列表-将包添加到包，将包添加到安装包，将包删除到卸载包](http://nuget.codeplex.com/workitem/155)
...
* [如果运行不带源的列表包，则会收到空引用错误](http://nuget.codeplex.com/workitem/164)
...
* [&lt;configsections&gt;，配置转换不能正常工作](http://nuget.codeplex.com/workitem/194)
...
* [Config 转换似乎在 web.config 文件中不起作用](http://nuget.codeplex.com/workitem/229)
* [init. ps1 未在自定义包上触发](http://nuget.codeplex.com/workitem/237)
...
* [默认情况下，已安装包的 "添加包" 对话框](http://nuget.codeplex.com/workitem/254)
...
* [创建谓词的别名-名词 cmdlet](http://nuget.codeplex.com/workitem/4)
...
* [将 Apache 标头添加到版权所有 .txt 文件](http://nuget.codeplex.com/workitem/36)
* [删除 Register-packagesource 命令](http://nuget.codeplex.com/workitem/37)
* [加载配置文件时程序包管理器不可用引发异常](http://nuget.codeplex.com/workitem/39)
* [init. ps1，install. ps1 和 uninstall 需要接收附加状态](http://nuget.codeplex.com/workitem/41)
...
* [将列表包的本地参数更改为已安装](http://nuget.codeplex.com/workitem/129)
* [将包重命名为 NuPack](http://nuget.codeplex.com/workitem/132)
