...
- 保留包 ID 前缀：许多包所有者已应用并授予了[保留包 ID 前缀](../reference/id-prefix-reservation.md)。 如果 [nuget.org](https://www.nuget.org/) 上或 Visual Studio 中的包 ID 旁有视觉对象复选标记，这意味着包所有者已满足我们的 ID 前缀保留[条件](../reference/id-prefix-reservation.md#id-prefix-reservation-criteria)。 这意味着包所有者清楚了解如何标识自己及其包。
