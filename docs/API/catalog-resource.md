...
名称            | 类型    | 必需 | 说明
--------------- | ------- | -------- | -----
@id             | string  | 是      | 到提取目录页 URL
commitId        | string  | 是      | 在此页中的最新提交与关联的唯一 ID
...
nuget:id        | string  | 是      | 与此叶包 ID
nuget:version   | string  | 是      | 与此叶包版本
...
目录： commitId        | string                     | 是      | 与此目录项关联的提交 ID
目录： commitTimeStamp | string                     | 是      | 此目录项的提交时间戳
id                      | string                     | 是      | 目录项的包 ID
发布               | string                     | 是      | 包的目录项目发布的日期
版本                 | string                     | 是      | 目录项的包版本
...
----------------------- | -------------------------- | -------- | -----
作者                 | string                     | no       |
created                 | string                     | 是      | 首次创建包的时间戳
dependencyGroups        | 对象的数组           | no       | 相同的格式设置为[package 元数据资源](registration-base-url-resource.md#package-dependency-group)
说明             | string                     | no       |
iconUrl                 | string                     | no       |
isPrerelease            | boolean                    | 是      | 是否预发布的包版本
语言                | string                     | no       |
licenseUrl              | string                     | no       |
列出                  | boolean                    | no       | 该程序包是否将列
MinClientVersion        | string                     | no       |
packageHash             | string                     | 是      | 使用编码的包的哈希[标准 base 64](https://tools.ietf.org/html/rfc4648#section-4)
packageHashAlgorithm    | string                     | 是      |
packageSize             | 整数                    | 是      | 包.nupkg 以字节为单位的大小
projectUrl              | string                     | no       |
releaseNotes            | string                     | no       |
requireLicenseAgreement | boolean                    | no       | 假定`false`如果排除
摘要                 | string                     | no       |
标记                    | 字符串数组           | no       |
标题                   | string                     | no       |
verbatimVersion         | string                     | no       | 因为它的版本字符串最初位于.nuspec
