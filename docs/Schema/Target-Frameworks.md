...
- [.nuspec 清单](../schema/nuspec.md)：根据项目的目标框架，包可以指示要包含在项目中的不同包。
...
- [packages.config](../Schema/packages-config.md)：依赖项的 `targetframework` 特性指定要安装的包的变体。
- [project.json](../Schema/project-json.md)：`frameworks` 节点指定可编译项目的框架版本。
...
> [!Warning]
> 建议不要使用 PCL。 尽管支持 PCL，但包创建者反而应支持 netstandard。 .NET 平台标准是 PCL 的一种演变，表示跨平台的二进制可移植性，它使用不绑定到静态的单个名字对象（如 portable-a+b+c 名字对象）。
