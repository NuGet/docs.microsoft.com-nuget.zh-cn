...
- [包源节](#package-source-sections)：-[packageSources](#packagesources)
...
注意：`dependencyVersion` 和 `repositoryPath` 仅适用于使用 `packages.config` 的项目。 `globalPackagesFolder` 仅适用于使用 `project.json` 和 PackageReference 格式的项目。
