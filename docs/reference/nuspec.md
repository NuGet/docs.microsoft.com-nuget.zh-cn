...
- <span data-ttu-id="5f799-117">`.nuspec` `nuget.exe pack` 对于使用的非 SDK 样式项目，请使用 `packages.config` 。</span><span class="sxs-lookup"><span data-stu-id="5f799-117">Use `.nuspec` with `nuget.exe pack` for non-SDK-style projects that use `packages.config`.</span></span>
...
<span data-ttu-id="5f799-145">将包上传到 nuget.org 时， `version` 字段的长度限制为64个字符。</span><span class="sxs-lookup"><span data-stu-id="5f799-145">When uploading a package to nuget.org, the `version` field is limited to 64 characters.</span></span>
...
<span data-ttu-id="5f799-159">将包上传到 nuget.org 时， `projectUrl` 字段的长度限制为4000个字符。</span><span class="sxs-lookup"><span data-stu-id="5f799-159">When uploading a package to nuget.org, the `projectUrl` field is limited to 4000 characters.</span></span>
...
<span data-ttu-id="5f799-182">将包上传到 nuget.org 时， `iconUrl` 字段的长度限制为4000个字符。</span><span class="sxs-lookup"><span data-stu-id="5f799-182">When uploading a package to nuget.org, the `iconUrl` field is limited to 4000 characters.</span></span>
...
#### <a name="readme"></a><span data-ttu-id="5f799-194">自述文件</span><span class="sxs-lookup"><span data-stu-id="5f799-194">readme</span></span>
...
#### <a name="summary"></a><span data-ttu-id="5f799-206">摘要</span><span class="sxs-lookup"><span data-stu-id="5f799-206">summary</span></span>
> [!Important]
...
<span data-ttu-id="5f799-223">将包上传到 nuget.org 时， `tags` 字段的长度限制为4000个字符。</span><span class="sxs-lookup"><span data-stu-id="5f799-223">When uploading a package to nuget.org, the `tags` field is limited to 4000 characters.</span></span>
...
<span data-ttu-id="5f799-237">将包上传到 nuget.org 时， `title` 字段限制为256个字符，但不用于任何显示目的。</span><span class="sxs-lookup"><span data-stu-id="5f799-237">When uploading a package to nuget.org, the `title` field is limited to 256 characters but is not used for any display purposes.</span></span>
...
#### <a name="contentfiles"></a><span data-ttu-id="5f799-257">contentFiles</span><span class="sxs-lookup"><span data-stu-id="5f799-257">contentFiles</span></span>
...
| <span data-ttu-id="5f799-282">标记</span><span class="sxs-lookup"><span data-stu-id="5f799-282">Token</span></span> | <span data-ttu-id="5f799-283">值源</span><span class="sxs-lookup"><span data-stu-id="5f799-283">Value source</span></span> | <span data-ttu-id="5f799-284">值</span><span class="sxs-lookup"><span data-stu-id="5f799-284">Value</span></span>
| --- | --- | ---
...
| <span data-ttu-id="5f799-314">特性</span><span class="sxs-lookup"><span data-stu-id="5f799-314">Attribute</span></span> | <span data-ttu-id="5f799-315">说明</span><span class="sxs-lookup"><span data-stu-id="5f799-315">Description</span></span> |
| --- | --- |
...
<span data-ttu-id="5f799-347">以下行指示相同包上的依赖项，但指定包括 `PackageA` 的 `contentFiles` 和 `build` 文件夹，以及除 `PackageB` 的 `native` 和 `compile` 文件夹以外的所有文件夹。</span><span class="sxs-lookup"><span data-stu-id="5f799-347">The following lines indicate dependencies on the same packages, but specify to include the `contentFiles` and `build` folders of `PackageA` and everything but the `native` and `compile` folders of `PackageB`"</span></span>
...
<span data-ttu-id="5f799-361">以下示例显示了 `<group>` 元素的不同变体：</span><span class="sxs-lookup"><span data-stu-id="5f799-361">The following example shows different variations of the `<group>` element:</span></span>
...
<span data-ttu-id="5f799-368">作为单个简单列表的替代方法，可使用 `<references>` 中的 `<group>` 元素根据目标项目的框架配置文件指定引用。</span><span class="sxs-lookup"><span data-stu-id="5f799-368">As an alternative to a single flat list, references can be specified according to the framework profile of the target project using `<group>` elements within `<references>`.</span></span>
...
<span data-ttu-id="5f799-379">`<frameworkAssemblies>` 元素包含零个或多个 `<frameworkAssembly>` 元素，这些元素指定以下特性：</span><span class="sxs-lookup"><span data-stu-id="5f799-379">The `<frameworkAssemblies>` element contains zero or more `<frameworkAssembly>` elements, each of which specifies the following attributes:</span></span>
...
<span data-ttu-id="5f799-416">**使用通配符的 DLL 集**</span><span class="sxs-lookup"><span data-stu-id="5f799-416">**Set of DLLs using a wildcard**</span></span>
...
<span data-ttu-id="5f799-420">内容文件是包需要包含在项目中的不可变文件。</span><span class="sxs-lookup"><span data-stu-id="5f799-420">Content files are immutable files that a package needs to include in a project.</span></span> <span data-ttu-id="5f799-421">不可变文件指的是使用项目不会修改的文件。</span><span class="sxs-lookup"><span data-stu-id="5f799-421">Being immutable, they are not intended to be modified by the consuming project.</span></span> <span data-ttu-id="5f799-422">内容文件示例包括：</span><span class="sxs-lookup"><span data-stu-id="5f799-422">Example content files include:</span></span>
...
<span data-ttu-id="5f799-433">**具有目录结构的内容文件**</span><span class="sxs-lookup"><span data-stu-id="5f799-433">**Content files with directory structure**</span></span>
...
<span data-ttu-id="5f799-437">**不具有扩展名的内容文件**</span><span class="sxs-lookup"><span data-stu-id="5f799-437">**Content files without extensions**</span></span>

<span data-ttu-id="5f799-438">若要包含不具有扩展名的文件，请使用 `*` 或 `**` 通配符：</span><span class="sxs-lookup"><span data-stu-id="5f799-438">To include files without an extension, use the `*` or `**` wildcards:</span></span>
...
<span data-ttu-id="5f799-439">**具有深层路径和深层目标的内容文件**</span><span class="sxs-lookup"><span data-stu-id="5f799-439">**Content files with deep path and deep target**</span></span>

<span data-ttu-id="5f799-440">在此情况下，由于源文件和目标文件的扩展名匹配，NuGet 假定目标是文件名而不是文件夹：</span><span class="sxs-lookup"><span data-stu-id="5f799-440">In this case, because the file extensions of the source and target match, NuGet assumes that the target is a file name and not a folder:</span></span>
...
<span data-ttu-id="5f799-441">**重命名包中的内容文件**</span><span class="sxs-lookup"><span data-stu-id="5f799-441">**Renaming a content file in the package**</span></span>
...
<span data-ttu-id="5f799-442">**排除文件**</span><span class="sxs-lookup"><span data-stu-id="5f799-442">**Excluding files**</span></span>
...
### <a name="using-the-contentfiles-element-for-content-files"></a><span data-ttu-id="5f799-443">对内容文件使用 contentFiles 元素</span><span class="sxs-lookup"><span data-stu-id="5f799-443">Using the contentFiles element for content files</span></span>

<span data-ttu-id="5f799-444">*NuGet 4.0+ 与 PackageReference*</span><span class="sxs-lookup"><span data-stu-id="5f799-444">*NuGet 4.0+ with PackageReference*</span></span>

<span data-ttu-id="5f799-445">默认情况下，包将内容放置在 `contentFiles` 文件夹中（见下文），`nuget pack` 使用默认特性将全部文件包含在该文件夹中。</span><span class="sxs-lookup"><span data-stu-id="5f799-445">By default, a package places content in a `contentFiles` folder (see below) and `nuget pack` included all files in that folder using default attributes.</span></span> <span data-ttu-id="5f799-446">在此情况下，根本没有必要在 `.nuspec` 中包含 `contentFiles` 节点。</span><span class="sxs-lookup"><span data-stu-id="5f799-446">In this case it's not necessary to include a `contentFiles` node in the `.nuspec` at all.</span></span>

<span data-ttu-id="5f799-447">若要控制包含哪些文件，`<contentFiles>` 元素指定是 `<files>` 元素的集合，可标识包含的确切文件。</span><span class="sxs-lookup"><span data-stu-id="5f799-447">To control which files are included, the `<contentFiles>` element specifies is a collection of `<files>` elements that identify the exact files include.</span></span>

<span data-ttu-id="5f799-448">这些文件用一组特性指定，用于描述如何在项目系统中使用这些文件：</span><span class="sxs-lookup"><span data-stu-id="5f799-448">These files are specified with a set of attributes that describe how they should be used within the project system:</span></span>

| <span data-ttu-id="5f799-449">特性</span><span class="sxs-lookup"><span data-stu-id="5f799-449">Attribute</span></span> | <span data-ttu-id="5f799-450">说明</span><span class="sxs-lookup"><span data-stu-id="5f799-450">Description</span></span> |
| --- | --- |
| <span data-ttu-id="5f799-451">**包括**</span><span class="sxs-lookup"><span data-stu-id="5f799-451">**include**</span></span> | <span data-ttu-id="5f799-452">（必需）文件或要包含的文件位置，受 `exclude` 特性指定的排除规则约束。</span><span class="sxs-lookup"><span data-stu-id="5f799-452">(Required) The location of the file or files to include, subject to exclusions specified by the `exclude` attribute.</span></span> <span data-ttu-id="5f799-453">路径相对于 `contentFiles` 文件夹，除非指定了绝对路径。</span><span class="sxs-lookup"><span data-stu-id="5f799-453">The path is relative to the `contentFiles` folder unless an absolute path is specified.</span></span> <span data-ttu-id="5f799-454">允许使用通配符 `*`，双通配符 `**` 意味着递归文件夹搜索。</span><span class="sxs-lookup"><span data-stu-id="5f799-454">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |
| <span data-ttu-id="5f799-455">**延伸**</span><span class="sxs-lookup"><span data-stu-id="5f799-455">**exclude**</span></span> | <span data-ttu-id="5f799-456">要从 `src` 位置排除的文件或文件模式的分号分隔列表。</span><span class="sxs-lookup"><span data-stu-id="5f799-456">A semicolon-delimited list of files or file patterns to exclude from the `src` location.</span></span> <span data-ttu-id="5f799-457">允许使用通配符 `*`，双通配符 `**` 意味着递归文件夹搜索。</span><span class="sxs-lookup"><span data-stu-id="5f799-457">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |
| <span data-ttu-id="5f799-458">**buildAction**</span><span class="sxs-lookup"><span data-stu-id="5f799-458">**buildAction**</span></span> | <span data-ttu-id="5f799-459">要分配给 MSBuild 的内容项的生成操作，如、、、等 `Content` `None` `Embedded Resource` `Compile` 。默认值为 `Compile` 。</span><span class="sxs-lookup"><span data-stu-id="5f799-459">The build action to assign to the content item for MSBuild, such as `Content`, `None`, `Embedded Resource`, `Compile`, etc. The default is `Compile`.</span></span> |
| <span data-ttu-id="5f799-460">**copyToOutput**</span><span class="sxs-lookup"><span data-stu-id="5f799-460">**copyToOutput**</span></span> | <span data-ttu-id="5f799-461">一个布尔值，该值指示是将内容项复制到生成 (还是发布) 输出文件夹。</span><span class="sxs-lookup"><span data-stu-id="5f799-461">A Boolean indicating whether to copy content items to the build (or publish) output folder.</span></span> <span data-ttu-id="5f799-462">默认值为 false。</span><span class="sxs-lookup"><span data-stu-id="5f799-462">The default is false.</span></span> |
| <span data-ttu-id="5f799-463">**flatten**</span><span class="sxs-lookup"><span data-stu-id="5f799-463">**flatten**</span></span> | <span data-ttu-id="5f799-464">一个布尔值，用于指示是将内容项复制到生成输出中的单个文件夹 (true)，还是保留包中的文件夹结构 (false)。</span><span class="sxs-lookup"><span data-stu-id="5f799-464">A Boolean indicating whether to copy content items to a single folder in the build output (true), or to preserve the folder structure in the package (false).</span></span> <span data-ttu-id="5f799-465">此标志仅在 copyToOutput 标志设置为 true 时才有效。</span><span class="sxs-lookup"><span data-stu-id="5f799-465">This flag only works when copyToOutput flag is set to true.</span></span> <span data-ttu-id="5f799-466">默认值为 false。</span><span class="sxs-lookup"><span data-stu-id="5f799-466">The default is false.</span></span> |

<span data-ttu-id="5f799-467">安装包时，NuGet 从上到下应用 `<contentFiles>` 的子元素。</span><span class="sxs-lookup"><span data-stu-id="5f799-467">When installing a package, NuGet applies the child elements of `<contentFiles>` from top to bottom.</span></span> <span data-ttu-id="5f799-468">如果多个条目与相同的文件匹配，那么应用全部条目。</span><span class="sxs-lookup"><span data-stu-id="5f799-468">If multiple entries match the same file then all entries are applied.</span></span> <span data-ttu-id="5f799-469">如果相同特性发生冲突，则最上面的条目将替代靠下的条目。</span><span class="sxs-lookup"><span data-stu-id="5f799-469">The top-most entry overrides the lower entries if there is a conflict for the same attribute.</span></span>

#### <a name="package-folder-structure"></a><span data-ttu-id="5f799-470">包文件夹结构</span><span class="sxs-lookup"><span data-stu-id="5f799-470">Package folder structure</span></span>

<span data-ttu-id="5f799-471">包项目应使用以下模式构建内容结构：</span><span class="sxs-lookup"><span data-stu-id="5f799-471">The package project should structure content using the following pattern:</span></span>

```
/contentFiles/{codeLanguage}/{TxM}/{any?}
```

- <span data-ttu-id="5f799-472">`codeLanguages` 可以是 `cs`、`vb`、`fs`、`any` 或给定 `$(ProjectLanguage)` 的小写等效形式</span><span class="sxs-lookup"><span data-stu-id="5f799-472">`codeLanguages` may be `cs`, `vb`, `fs`, `any`, or the lowercase equivalent of a given `$(ProjectLanguage)`</span></span>
- <span data-ttu-id="5f799-473">`TxM` 是 NuGet 支持的任何合法目标框架名字对象（请参阅[目标框架](../reference/target-frameworks.md)）。</span><span class="sxs-lookup"><span data-stu-id="5f799-473">`TxM` is any legal target framework moniker that NuGet supports (see [Target frameworks](../reference/target-frameworks.md)).</span></span>
- <span data-ttu-id="5f799-474">任何文件夹结构都可以附加到此语法的末尾。</span><span class="sxs-lookup"><span data-stu-id="5f799-474">Any folder structure may be appended to the end of this syntax.</span></span>

<span data-ttu-id="5f799-475">例如：</span><span class="sxs-lookup"><span data-stu-id="5f799-475">For example:</span></span>
...
<span data-ttu-id="5f799-476">空文件夹可以使用 `.` 来选择不提供特定语言和 TxM 组合的内容，例如：</span><span class="sxs-lookup"><span data-stu-id="5f799-476">Empty folders can use `.` to opt out of providing content for certain combinations of language and TxM, for example:</span></span>
...
#### <a name="example-contentfiles-section"></a><span data-ttu-id="5f799-477">contentFiles 部分示例</span><span class="sxs-lookup"><span data-stu-id="5f799-477">Example contentFiles section</span></span>
...
## <a name="framework-reference-groups"></a><span data-ttu-id="5f799-478">框架引用组</span><span class="sxs-lookup"><span data-stu-id="5f799-478">Framework reference groups</span></span>

<span data-ttu-id="5f799-479">*仅限版本 5.1 + 同时 PackageReference*</span><span class="sxs-lookup"><span data-stu-id="5f799-479">*Version 5.1+ wih PackageReference only*</span></span>

<span data-ttu-id="5f799-480">框架引用是一个 .NET Core 概念，表示 WPF 或 Windows 窗体等共享框架。</span><span class="sxs-lookup"><span data-stu-id="5f799-480">Framework References are a .NET Core concept representing shared frameworks such as WPF or Windows Forms.</span></span>
<span data-ttu-id="5f799-481">通过指定共享框架，包可确保其所有框架依赖项都包括在引用项目中。</span><span class="sxs-lookup"><span data-stu-id="5f799-481">By specifying a shared framework, the package ensures that all its framework dependencies are included in the referencing project.</span></span>

<span data-ttu-id="5f799-482">每个 `<group>` 元素都需要一个 `targetFramework` 属性和零个或多个 `<frameworkReference>` 元素。</span><span class="sxs-lookup"><span data-stu-id="5f799-482">Each `<group>` element requires a `targetFramework` attribute and zero or more `<frameworkReference>` elements.</span></span>

<span data-ttu-id="5f799-483">下面的示例演示如何为 .NET Core WPF 项目生成 nuspec。</span><span class="sxs-lookup"><span data-stu-id="5f799-483">The following example shows a nuspec generated for a .NET Core WPF project.</span></span>
<span data-ttu-id="5f799-484">请注意，不建议使用包含框架引用的手动创作 nuspecs。</span><span class="sxs-lookup"><span data-stu-id="5f799-484">Note that hand authoring nuspecs that contain framework references is not recommended.</span></span> <span data-ttu-id="5f799-485">请考虑改用 [目标](msbuild-targets.md) 包，这会自动从项目推断它们。</span><span class="sxs-lookup"><span data-stu-id="5f799-485">Consider using the [targets](msbuild-targets.md) pack instead, which will automatically infer them from the project.</span></span>
...
## <a name="example-nuspec-files"></a><span data-ttu-id="5f799-486">示例 nuspec 文件</span><span class="sxs-lookup"><span data-stu-id="5f799-486">Example nuspec files</span></span>

<span data-ttu-id="5f799-487">**未指定依赖项或文件的简单 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="5f799-487">**A simple `.nuspec` that does not specify dependencies or files**</span></span>
...
<span data-ttu-id="5f799-488">**具有依赖项的 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="5f799-488">**A `.nuspec` with dependencies**</span></span>
...
<span data-ttu-id="5f799-489">**具有文件的 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="5f799-489">**A `.nuspec` with files**</span></span>
...
<span data-ttu-id="5f799-490">**具有 Framework 程序集的 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="5f799-490">**A `.nuspec` with framework assemblies**</span></span>
...
<span data-ttu-id="5f799-491">在此示例中，为特定的项目目标安装了以下内容：</span><span class="sxs-lookup"><span data-stu-id="5f799-491">In this example, the following are installed for specific project targets:</span></span>

- <span data-ttu-id="5f799-492">.NET4 -> `System.Web``System.Net`</span><span class="sxs-lookup"><span data-stu-id="5f799-492">.NET4 -> `System.Web`, `System.Net`</span></span>
- <span data-ttu-id="5f799-493">.NET4 Client Profile -> `System.Net`</span><span class="sxs-lookup"><span data-stu-id="5f799-493">.NET4 Client Profile -> `System.Net`</span></span>
- <span data-ttu-id="5f799-494">Silverlight 3 -> `System.Json`</span><span class="sxs-lookup"><span data-stu-id="5f799-494">Silverlight 3 -> `System.Json`</span></span>
- <span data-ttu-id="5f799-495">WindowsPhone -> `Microsoft.Devices.Sensors`</span><span class="sxs-lookup"><span data-stu-id="5f799-495">WindowsPhone -> `Microsoft.Devices.Sensors`</span></span>
