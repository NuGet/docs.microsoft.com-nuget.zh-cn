...
- <span data-ttu-id="f341a-117">`.nuspec` `nuget.exe pack` 对于使用的非 SDK 样式项目，请使用 `packages.config` 。</span><span class="sxs-lookup"><span data-stu-id="f341a-117">Use `.nuspec` with `nuget.exe pack` for non-SDK-style projects that use `packages.config`.</span></span>
...
<span data-ttu-id="f341a-145">将包上传到 nuget.org 时， `version` 字段的长度限制为64个字符。</span><span class="sxs-lookup"><span data-stu-id="f341a-145">When uploading a package to nuget.org, the `version` field is limited to 64 characters.</span></span>
...
<span data-ttu-id="f341a-161">包的许可证的 URL，通常显示在 Ui （如 nuget.org）中。</span><span class="sxs-lookup"><span data-stu-id="f341a-161">A URL for the package's license, often shown in UIs like nuget.org.</span></span>
...
<span data-ttu-id="f341a-172">对于 MSBuild 等效项，请查看 [打包许可证表达式或许可证文件](msbuild-targets.md#packing-a-license-expression-or-a-license-file)。</span><span class="sxs-lookup"><span data-stu-id="f341a-172">For the MSBuild equivalent, take a look at [Packing a license expression or a license file](msbuild-targets.md#packing-a-license-expression-or-a-license-file).</span></span>
...
#### <a name="tags"></a><span data-ttu-id="f341a-213">标记</span><span class="sxs-lookup"><span data-stu-id="f341a-213">tags</span></span>
...
#### <a name="title"></a><span data-ttu-id="f341a-226">title</span><span class="sxs-lookup"><span data-stu-id="f341a-226">title</span></span>
...
<span data-ttu-id="f341a-302">在 MSBuild 中生成具有 `Release` 配置且 `AssemblyName` 为 `LoggingLibrary` 的程序集，包中 `.nuspec` 文件中的结果行如下所示：</span><span class="sxs-lookup"><span data-stu-id="f341a-302">And you build an assembly whose `AssemblyName` is `LoggingLibrary` with the `Release` configuration in MSBuild, the resulting lines in the `.nuspec` file in the package is as follows:</span></span>
...
| <span data-ttu-id="f341a-330">build</span><span class="sxs-lookup"><span data-stu-id="f341a-330">build</span></span> | <span data-ttu-id="f341a-331">生成（MSBuild 属性和目标）</span><span class="sxs-lookup"><span data-stu-id="f341a-331">build (MSBuild props and targets)</span></span> |
...
<span data-ttu-id="f341a-434">**排除文件**</span><span class="sxs-lookup"><span data-stu-id="f341a-434">**Excluding files**</span></span>
...
### <a name="using-the-contentfiles-element-for-content-files"></a><span data-ttu-id="f341a-435">对内容文件使用 contentFiles 元素</span><span class="sxs-lookup"><span data-stu-id="f341a-435">Using the contentFiles element for content files</span></span>

<span data-ttu-id="f341a-436">*NuGet 4.0+ 与 PackageReference*</span><span class="sxs-lookup"><span data-stu-id="f341a-436">*NuGet 4.0+ with PackageReference*</span></span>

<span data-ttu-id="f341a-437">默认情况下，包将内容放置在 `contentFiles` 文件夹中（见下文），`nuget pack` 使用默认特性将全部文件包含在该文件夹中。</span><span class="sxs-lookup"><span data-stu-id="f341a-437">By default, a package places content in a `contentFiles` folder (see below) and `nuget pack` included all files in that folder using default attributes.</span></span> <span data-ttu-id="f341a-438">在此情况下，根本没有必要在 `.nuspec` 中包含 `contentFiles` 节点。</span><span class="sxs-lookup"><span data-stu-id="f341a-438">In this case it's not necessary to include a `contentFiles` node in the `.nuspec` at all.</span></span>

<span data-ttu-id="f341a-439">若要控制包含哪些文件，`<contentFiles>` 元素指定是 `<files>` 元素的集合，可标识包含的确切文件。</span><span class="sxs-lookup"><span data-stu-id="f341a-439">To control which files are included, the `<contentFiles>` element specifies is a collection of `<files>` elements that identify the exact files include.</span></span>

<span data-ttu-id="f341a-440">这些文件用一组特性指定，用于描述如何在项目系统中使用这些文件：</span><span class="sxs-lookup"><span data-stu-id="f341a-440">These files are specified with a set of attributes that describe how they should be used within the project system:</span></span>

| <span data-ttu-id="f341a-441">Attribute</span><span class="sxs-lookup"><span data-stu-id="f341a-441">Attribute</span></span> | <span data-ttu-id="f341a-442">说明</span><span class="sxs-lookup"><span data-stu-id="f341a-442">Description</span></span> |
| --- | --- |
| <span data-ttu-id="f341a-443">**包括**</span><span class="sxs-lookup"><span data-stu-id="f341a-443">**include**</span></span> | <span data-ttu-id="f341a-444">（必需）文件或要包含的文件位置，受 `exclude` 特性指定的排除规则约束。</span><span class="sxs-lookup"><span data-stu-id="f341a-444">(Required) The location of the file or files to include, subject to exclusions specified by the `exclude` attribute.</span></span> <span data-ttu-id="f341a-445">路径相对于 `contentFiles` 文件夹，除非指定了绝对路径。</span><span class="sxs-lookup"><span data-stu-id="f341a-445">The path is relative to the `contentFiles` folder unless an absolute path is specified.</span></span> <span data-ttu-id="f341a-446">允许使用通配符 `*`，双通配符 `**` 意味着递归文件夹搜索。</span><span class="sxs-lookup"><span data-stu-id="f341a-446">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |
| <span data-ttu-id="f341a-447">**排除**</span><span class="sxs-lookup"><span data-stu-id="f341a-447">**exclude**</span></span> | <span data-ttu-id="f341a-448">要从 `src` 位置排除的文件或文件模式的分号分隔列表。</span><span class="sxs-lookup"><span data-stu-id="f341a-448">A semicolon-delimited list of files or file patterns to exclude from the `src` location.</span></span> <span data-ttu-id="f341a-449">允许使用通配符 `*`，双通配符 `**` 意味着递归文件夹搜索。</span><span class="sxs-lookup"><span data-stu-id="f341a-449">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |
| <span data-ttu-id="f341a-450">**buildAction**</span><span class="sxs-lookup"><span data-stu-id="f341a-450">**buildAction**</span></span> | <span data-ttu-id="f341a-451">要分配给 MSBuild 的内容项的生成操作，如、、、等 `Content` `None` `Embedded Resource` `Compile` 。默认值为 `Compile` 。</span><span class="sxs-lookup"><span data-stu-id="f341a-451">The build action to assign to the content item for MSBuild, such as `Content`, `None`, `Embedded Resource`, `Compile`, etc. The default is `Compile`.</span></span> |
| <span data-ttu-id="f341a-452">**copyToOutput**</span><span class="sxs-lookup"><span data-stu-id="f341a-452">**copyToOutput**</span></span> | <span data-ttu-id="f341a-453">一个布尔值，该值指示是将内容项复制到生成 (还是发布) 输出文件夹。</span><span class="sxs-lookup"><span data-stu-id="f341a-453">A Boolean indicating whether to copy content items to the build (or publish) output folder.</span></span> <span data-ttu-id="f341a-454">默认值为 false。</span><span class="sxs-lookup"><span data-stu-id="f341a-454">The default is false.</span></span> |
| <span data-ttu-id="f341a-455">**flatten**</span><span class="sxs-lookup"><span data-stu-id="f341a-455">**flatten**</span></span> | <span data-ttu-id="f341a-456">一个布尔值，用于指示是将内容项复制到生成输出中的单个文件夹 (true)，还是保留包中的文件夹结构 (false)。</span><span class="sxs-lookup"><span data-stu-id="f341a-456">A Boolean indicating whether to copy content items to a single folder in the build output (true), or to preserve the folder structure in the package (false).</span></span> <span data-ttu-id="f341a-457">此标志仅在 copyToOutput 标志设置为 true 时才有效。</span><span class="sxs-lookup"><span data-stu-id="f341a-457">This flag only works when copyToOutput flag is set to true.</span></span> <span data-ttu-id="f341a-458">默认值为 false。</span><span class="sxs-lookup"><span data-stu-id="f341a-458">The default is false.</span></span> |

<span data-ttu-id="f341a-459">安装包时，NuGet 从上到下应用 `<contentFiles>` 的子元素。</span><span class="sxs-lookup"><span data-stu-id="f341a-459">When installing a package, NuGet applies the child elements of `<contentFiles>` from top to bottom.</span></span> <span data-ttu-id="f341a-460">如果多个条目与相同的文件匹配，那么应用全部条目。</span><span class="sxs-lookup"><span data-stu-id="f341a-460">If multiple entries match the same file then all entries are applied.</span></span> <span data-ttu-id="f341a-461">如果相同特性发生冲突，则最上面的条目将替代靠下的条目。</span><span class="sxs-lookup"><span data-stu-id="f341a-461">The top-most entry overrides the lower entries if there is a conflict for the same attribute.</span></span>

#### <a name="package-folder-structure"></a><span data-ttu-id="f341a-462">包文件夹结构</span><span class="sxs-lookup"><span data-stu-id="f341a-462">Package folder structure</span></span>

<span data-ttu-id="f341a-463">包项目应使用以下模式构建内容结构：</span><span class="sxs-lookup"><span data-stu-id="f341a-463">The package project should structure content using the following pattern:</span></span>

    /contentFiles/{codeLanguage}/{TxM}/{any?}

- <span data-ttu-id="f341a-464">`codeLanguages` 可以是 `cs`、`vb`、`fs`、`any` 或给定 `$(ProjectLanguage)` 的小写等效形式</span><span class="sxs-lookup"><span data-stu-id="f341a-464">`codeLanguages` may be `cs`, `vb`, `fs`, `any`, or the lowercase equivalent of a given `$(ProjectLanguage)`</span></span>
- <span data-ttu-id="f341a-465">`TxM` 是 NuGet 支持的任何合法目标框架名字对象（请参阅[目标框架](../reference/target-frameworks.md)）。</span><span class="sxs-lookup"><span data-stu-id="f341a-465">`TxM` is any legal target framework moniker that NuGet supports (see [Target frameworks](../reference/target-frameworks.md)).</span></span>
- <span data-ttu-id="f341a-466">任何文件夹结构都可以附加到此语法的末尾。</span><span class="sxs-lookup"><span data-stu-id="f341a-466">Any folder structure may be appended to the end of this syntax.</span></span>

<span data-ttu-id="f341a-467">例如：</span><span class="sxs-lookup"><span data-stu-id="f341a-467">For example:</span></span>
...
<span data-ttu-id="f341a-468">空文件夹可以使用 `.` 来选择不提供特定语言和 TxM 组合的内容，例如：</span><span class="sxs-lookup"><span data-stu-id="f341a-468">Empty folders can use `.` to opt out of providing content for certain combinations of language and TxM, for example:</span></span>

    /contentFiles/vb/any/code.vb
    /contentFiles/cs/any/.

#### <a name="example-contentfiles-section"></a><span data-ttu-id="f341a-469">contentFiles 部分示例</span><span class="sxs-lookup"><span data-stu-id="f341a-469">Example contentFiles section</span></span>
...
## <a name="framework-reference-groups"></a><span data-ttu-id="f341a-470">框架引用组</span><span class="sxs-lookup"><span data-stu-id="f341a-470">Framework reference groups</span></span>

<span data-ttu-id="f341a-471">*仅限版本 5.1 + 同时 PackageReference*</span><span class="sxs-lookup"><span data-stu-id="f341a-471">*Version 5.1+ wih PackageReference only*</span></span>

<span data-ttu-id="f341a-472">框架引用是一个 .NET Core 概念，表示 WPF 或 Windows 窗体等共享框架。</span><span class="sxs-lookup"><span data-stu-id="f341a-472">Framework References are a .NET Core concept representing shared frameworks such as WPF or Windows Forms.</span></span>
<span data-ttu-id="f341a-473">通过指定共享框架，包可确保其所有框架依赖项都包括在引用项目中。</span><span class="sxs-lookup"><span data-stu-id="f341a-473">By specifying a shared framework, the package ensures that all its framework dependencies are included in the referencing project.</span></span>

<span data-ttu-id="f341a-474">每个 `<group>` 元素都需要一个 `targetFramework` 属性和零个或多个 `<frameworkReference>` 元素。</span><span class="sxs-lookup"><span data-stu-id="f341a-474">Each `<group>` element requires a `targetFramework` attribute and zero or more `<frameworkReference>` elements.</span></span>

<span data-ttu-id="f341a-475">下面的示例演示如何为 .NET Core WPF 项目生成 nuspec。</span><span class="sxs-lookup"><span data-stu-id="f341a-475">The following example shows a nuspec generated for a .NET Core WPF project.</span></span>
<span data-ttu-id="f341a-476">请注意，不建议使用包含框架引用的手动创作 nuspecs。</span><span class="sxs-lookup"><span data-stu-id="f341a-476">Note that hand authoring nuspecs that contain framework references is not recommended.</span></span> <span data-ttu-id="f341a-477">请考虑改用 [目标](msbuild-targets.md) 包，这会自动从项目推断它们。</span><span class="sxs-lookup"><span data-stu-id="f341a-477">Consider using the [targets](msbuild-targets.md) pack instead, which will automatically infer them from the project.</span></span>
...
## <a name="example-nuspec-files"></a><span data-ttu-id="f341a-478">示例 nuspec 文件</span><span class="sxs-lookup"><span data-stu-id="f341a-478">Example nuspec files</span></span>

<span data-ttu-id="f341a-479">**未指定依赖项或文件的简单 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="f341a-479">**A simple `.nuspec` that does not specify dependencies or files**</span></span>
...
<span data-ttu-id="f341a-480">**具有依赖项的 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="f341a-480">**A `.nuspec` with dependencies**</span></span>
...
<span data-ttu-id="f341a-481">**具有文件的 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="f341a-481">**A `.nuspec` with files**</span></span>
...
<span data-ttu-id="f341a-482">**具有 Framework 程序集的 `.nuspec`**</span><span class="sxs-lookup"><span data-stu-id="f341a-482">**A `.nuspec` with framework assemblies**</span></span>
...
<span data-ttu-id="f341a-483">在此示例中，为特定的项目目标安装了以下内容：</span><span class="sxs-lookup"><span data-stu-id="f341a-483">In this example, the following are installed for specific project targets:</span></span>

- <span data-ttu-id="f341a-484">.NET4 -> `System.Web``System.Net`</span><span class="sxs-lookup"><span data-stu-id="f341a-484">.NET4 -> `System.Web`, `System.Net`</span></span>
- <span data-ttu-id="f341a-485">.NET4 Client Profile -> `System.Net`</span><span class="sxs-lookup"><span data-stu-id="f341a-485">.NET4 Client Profile -> `System.Net`</span></span>
- <span data-ttu-id="f341a-486">Silverlight 3 -> `System.Json`</span><span class="sxs-lookup"><span data-stu-id="f341a-486">Silverlight 3 -> `System.Json`</span></span>
- <span data-ttu-id="f341a-487">WindowsPhone -> `Microsoft.Devices.Sensors`</span><span class="sxs-lookup"><span data-stu-id="f341a-487">WindowsPhone -> `Microsoft.Devices.Sensors`</span></span>
