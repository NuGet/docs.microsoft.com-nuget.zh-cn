...
对于使用 `packages.config` 或 `project.json` 的项目，NuGet 支持在安装和卸载包时将包转换为源代码和配置文件的功能。
...
- （NuGet 2.6 及更高版本）在包的 `content` 文件夹中包含 `app.config.install.xdt` 和 `web.config.install.xdt` 文件，并使用 [XDT 语法](https://msdn.microsoft.com/library/dd465326.aspx)描述所需更改。 使用此选项还可以包含 `.uninstall.xdt` 文件，在从项目删除包时撤销更改。
