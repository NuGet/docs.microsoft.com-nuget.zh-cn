...
无论包是什么用途或者它包含什么代码，均使用 `nuget.exe` 将该功能打包进可以与任意数量的其他开发人员共享且可以由其使用的组件中。 若要安装 `nuget.exe`，请参阅[安装 NuGet CLI](../guides/Install-NuGet.md#nuget-cli)。 请注意，Visual Studio 不会自动包含 `nuget.exe`。

从技术角度来讲，NuGet 包仅仅是一个使用 `.nupkg` 扩展重命名且其中的内容与特定约定相匹配的 ZIP 文件。 本主题介绍创建满足这些约定的包的详细流程。 有关重点演练，请参阅[创建和发布包快速入门](../quickstart/create-and-publish-a-package.md)。
...
- [创建 `.nuspec` 文件](#creating-the-nuspec-file)，从：
...
- [选择唯一的包标识符并设置版本号](#choosing-a-unique-package-identifier-and-setting-the-version-number)
- [设置包类型](#setting-a-package-type)（NuGet 3.5 以及更高版本）
- [添加自述文件和其他文件](#adding-a-readme-and-other-files)
- [将 MSBuild 属性和目标包含到包中](#including-msbuild-props-and-targets-in-a-package)
- [创作包含 COM 互操作程序集的包](#authoring-packages-with-com-interop-assemblies)
- [运行 NuGet 包生成 .nupkg 文件](#running-nuget-pack-to-generate-the-nupkg-file)

完成这些核心步骤以后，可以集成多种其他功能（如本文档其他部分所述）。 请参阅以下[后续步骤](#next-steps)。

> [!Note]
> 本主题适用于除使用 Visual Studio 2017 和 NuGet 4.0+ 的 .NET Core 项目以外的项目类型。 在这些 .NET Core 项目中，NuGet 直接使用 `.csproj` 文件中的信息。 有关详细信息，请参阅[使用 Visual Studio 2017 创建 .NET Standard 包](../guides/create-net-standard-packages-vs2017.md)和[ NuGet 包和作为 MSBuild 目标还原](../schema/msbuild-targets.md)。
...
1. 如下所示，包含所需属性和可选属性。 有关确切的详细信息（包括此处未提及的其他属性），请参阅 [.nuspec 引用](../schema/nuspec.md)。
...
有关声明依赖项并指定版本号的详细信息，请参阅[包版本控制](../reference/package-versioning.md)。 还可以使用 `dependency` 元素上的 `include` 和 `exclude` 特性直接在包中结合依赖项的资产。 请参阅 [.nuspec 引用 - 依赖项](../Schema/nuspec.md#dependencies)。
...
| 生成 | MSBuild `.targets` 和 `.props` 文件 | 自动插入到项目文件 (NuGet 2.x) 或 `project.lock.json` (NuGet 3.x+)。 |
...
此令牌在打包时替换为项目文件的 `AssemblyName` 值。 有关项目值到 `.nuspec` 令牌的确切映射，请参阅[替换令牌引用](../schema/nuspec.md#replacement-tokens)。
...
包类型在 `.nuspec` 文件或 `project.json` 中设置。 在这两种情况下，后向兼容最好不显式设置 `Dependency` 类型，而是依赖 NuGet 在没有指定类型时假设此类型。
...
当 NuGet 2.x 使用 `\build` 文件安装包时，它在指向 `.targets` 和 `.props` 文件的项目文件中添加 MSBuild `<Import>` 元素。 （`.props` 添加到项目文件顶部；`.targets` 添加到底部。）
...
可以在 Visual Studio 中手动测试安装，或是使用常规[包安装步骤](../Quickstart/Use-a-Package.md)在命令行上测试。

对于自动测试，基本流程如下所示：

1. 将 `.nupkg` 文件复制到本地文件夹。
1. 使用 `nuget sources -name <name> -source <path>` 命令将文件夹添加到包源（请参阅 [nuget 源](../tools/cli-ref-sources.md)）。 请注意，只需在任意给定计算机上设置一次此本地源。
