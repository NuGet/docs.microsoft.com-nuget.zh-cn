...
1. 请注意，**包括预发行版**和**包源**卸载包时，控件将产生任何影响。
...
1. <a name="implicit_reference"></a>某些包的**更新**按钮处于禁用状态，并显示一条消息，指出，它"隐式引用由 SDK"（或"AutoReferenced"）。 消息指示包，如 Microsoft.NETCore.App 或 Microsoft.NETStandard.Library，是更大的框架或 SDK 的一部分，并且不应单独更新。 (此类包内部标记有`<IsImplicitlyDefined>True</IsImplicitlyDefined>`。)若要更新的包，更新其所属，推断从包名称包含的 SDK 的 SDK。 例如，如 Microsoft.NETCore.App 包是.NET 核心 SDK 的一部分，因此你需要将.NET 核心 SDK 安装更新。
...
在此示例中，ClassLibrary1 项目使用 EntityFramework 6.2.0，而 ConsoleApp1 正在使用 EntityFramework 6.1.0。 要合并包版本，请执行以下操作：
...
**强制卸载，即使在其上有依赖关系**： 当选中时，卸载程序包，即使它仍然引用项目中。 这通常与结合使用**删除依赖关系**若要删除包和任何依赖项安装它。 使用此选项可能，但是，会导致损坏的引用，项目中。 在这种情况下，你可能需要[重新安装这些其他包](../consume-packages/reinstalling-and-updating-packages.md)。
