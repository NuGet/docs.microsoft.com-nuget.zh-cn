...
多年以来，NuGet 添加了许多服务，所有这些服务都驻留于 `NuGet.VisualStudio.dll` 程序集中的 `NuGet.VisualStudio` 命名空间：

从 NuGet 3.3+ 开始，NuGet 可以导出以下

- [`IRegistryKey`](#iregistrykey-interface)：检索注册表子项中的值的方法。 (3.3+)
- [`IVsCredentialProvider`](#ivscredentialprovider-interface) 包含用于获取 NuGet 操作凭据的方法。 (4.0+)
- [`IVsFrameworkCompatibility`](#ivsframeworkcompatibility-interface) 包含用于发现框架和框架之间的兼容性的方法。 (4.0+)
- [`IVsFrameworkCompatibility2`](#ivsframeworkcompatibility2-interface) 包含用于发现框架和框架之间的兼容性的方法。 (4.0+)
- [`IVsFrameworkCompatibility3`](#ivsframeworkcompatibility3-interface) 包含用于发现框架和框架之间的兼容性的方法。 (5.8+)
- [`IVsFrameworkParser`](#ivsframeworkparser-interface) 用于处理字符串与 [FrameworkName](/dotnet/api/system.runtime.versioning.frameworkname) (4.0+) 之间的转换的接口
- [`IVsFrameworkParser2`](#ivsframeworkparser2-interface) 用于分析 .NET Framework 字符串的接口。 请参阅 [NuGet-IVsFrameworkParser](https://aka.ms/NuGet-IVsFrameworkParser)。 (5.8+)
- [`IVsGlobalPackagesInitScriptExecutor`](#ivsglobalpackagesinitscriptexecutor-interface) 从解决方案 (4.0+) 中的包执行 powershell 脚本
- [`IVsNuGetFramework`](#ivsnugetframework-interface) 表示 .NET 目标框架名字对象的组件的类型。 (5.8+)
- [`IVsPackageInstaller`](#ivspackageinstaller-interface)：将 NuGet 包安装到项目中的方法。 (3.3+)
- [`IVsPackageInstaller2](#ivspackageinstaller2-interface) 包含用于将单个包的最新版本安装到当前解决方案中的项目的方法。
- [`IVsPackageInstallerEvents`](#ivspackageinstallerevents-interface)：包安装/卸载的事件。 (3.3+)
- [`IVsPackageInstallerProjectEvents`](#ivspackageinstallerprojectevents-interface)：包安装/卸载的批处理事件。 (3.3+)
- [`IVsPackageInstallerServices`](#ivspackageinstallerservices-interface)：检索当前解决方案中安装的包及检查项目中是否已安装给定包的方法。 (3.3+)
- [`IVsPackageManagerProvider`](#ivspackagemanagerprovider-interface)：提供 NuGet 包的替代包管理器建议的方法。 (3.3+)
- [`IVsPackageMetadata`](#ivspackagemetadata-interface)：检索已安装包相关信息的方法。 (3.3+)
- [`IVsPackageProjectMetadata`](#ivspackageprojectmetadata-interface)：检索正在执行 NuGet 操作的项目相关信息的方法。 (3.3+)
- [`IVsPackageRestorer`](#ivspackagerestorer-interface)：还原项目中已安装包的方法。 (3.3+)
- [`IVsPackageSourceProvider`](#ivspackagesourceprovider-interface)：检索 NuGet 包源列表的方法。 (3.3+)
- [`IVsPackageUninstaller`](#ivspackageuninstaller-interface)：从项目中卸载 NuGet 包的方法。 (3.3+)
- [`IVsPathContext`](#ivspathcontext-interface) 特定于当前上下文（如项目上下文）的 NuGet 路径信息。 (4.0+)
- [`IVsPathContext2`](#ivspathcontext2-interface) 特定于当前上下文（如项目上下文）的 NuGet 路径信息。 (5.0+)
- [`IVsPathContextProvider`](#ivspathcontextprovider-interface) 用于初始化 [IVsPathContext](#ivspathcontext-interface) 实例的工厂。 (4.0+)
- [`IVsPathContextProvider2`](#ivspathcontextprovider2-interface) 用于初始化 [IVsPathContext2](#ivspathcontext2-interface) 实例的工厂。 (5.0+)
- [`IVsProjectJsonToPackageReferenceMigrateResult`](#ivsprojectjsontopackagereferencemigrateresult-interface) 包含对旧版项目执行迁移操作的结果 (4.3+)
- [`IVsProjectJsonToPackageReferenceMigrator`](#ivsprojectjsontopackagereferencemigrator-interface) 包含将基于 json 的旧版项目迁移到基于 PackageReference 的项目的方法。 (4.3+)
- [`IVsSemanticVersionComparer`](#ivssemanticversioncomparer-interface) 用于比较两个不透明版本字符串的接口，方法是将它们视为 NuGet 语义 (4.0+)
- [`IVsTemplateWizard`](#ivstemplatewizard-interface)：专为项目/项模板设计，用以包括预安装的包；此接口并非要从代码中调用，且没有任何公共方法。 (3.3+)
...
1. 要使用服务，请通过 [MEF 导入属性](/dotnet/framework/mef/index#imports-and-exports-with-attributes)，或通过[IComponentModel 服务](/dotnet/api/microsoft.visualstudio.componentmodelhost.icomponentmodel)将其导入。
...
对于引用，NuGet.VisualStudio 的源代码包含在 [NuGet.Clients 存储库](https://github.com/NuGet/NuGet.Client/tree/dev/src/NuGet.Clients/NuGet.VisualStudio)中。

## <a name="iregistrykey-interface"></a>IRegistryKey 接口
...
## <a name="ivscredentialprovider-interface"></a>IVsCredentialProvider 接口
...
## <a name="ivsframeworkcompatibility-interface"></a>IVsFrameworkCompatibility 接口
...
## <a name="ivsframeworkcompatibility2-interface"></a>IVsFrameworkCompatibility2 接口
...
## <a name="ivsframeworkcompatibility3-interface"></a>IVsFrameworkCompatibility3 接口
...
## <a name="ivsframeworkparser-interface"></a>IVsFrameworkParser 接口
...
## <a name="ivsframeworkparser2-interface"></a>IVsFrameworkParser2 接口
...
## <a name="ivsglobalpackagesinitscriptexecutor-interface"></a>IVsGlobalPackagesInitScriptExecutor 接口
...
## <a name="ivsnugetframework-interface"></a>IVsNuGetFramework 接口
...
## <a name="ivspackageinstaller-interface"></a>IVsPackageInstaller 接口
...
## <a name="ivspackageinstaller2-interface"></a>IVsPackageinstaller2 接口
...
## <a name="ivspackageinstallerevents-interface"></a>IVsPackageInstallerEvents 接口
...
## <a name="ivspackageinstallerprojectevents-interface"></a>IVsPackageInstallerProjectEvents 接口
...
## <a name="ivspackageinstallerservices-interface"></a>IVsPackageInstallerServices 接口
...
## <a name="ivspackagemanagerprovider-interface"></a>IVsPackageManagerProvider 接口
...
## <a name="ivspackagemetadata-interface"></a>IVsPackageMetadata 接口
...
## <a name="ivspackageprojectmetadata-interface"></a>IVsPackageProjectMetadata 接口
...
## <a name="ivspackagerestorer-interface"></a>IVsPackageRestorer 接口
...
## <a name="ivspackagesourceprovider-interface"></a>IVsPackageSourceProvider 接口
...
## <a name="ivspackageuninstaller-interface"></a>IVsPackageUninstaller 接口
...
## <a name="ivspathcontext-interface"></a>IVsPathContext 接口
...
## <a name="ivspathcontext2-interface"></a>IVsPathContext2 接口
...
## <a name="ivspathcontextprovider-interface"></a>IVsPathContextProvider 接口
...
## <a name="ivspathcontextprovider2-interface"></a>IVsPathContextProvider2 接口
...
## <a name="ivsprojectjsontopackagereferencemigrateresult-interface"></a>IVsProjectJsonToPackageReferenceMigrateResult 接口
...
## <a name="ivsprojectjsontopackagereferencemigrator-interface"></a>IVsProjectJsonToPackageReferenceMigrator 接口
...
## <a name="ivssemanticversioncomparer-interface"></a>IVsSemanticVersionComparer 接口
...
## <a name="ivstemplatewizard-interface"></a>IVsTemplateWizard 接口
